...
fn criterion_benchmark(c: &mut Criterion) {
	c.bench_function("bench read dir", |b| {
		b.iter(|| bench_read_dir(black_box("/run/media/mmcblk0p1/steamapps/")))
	});
	c.bench_function("bench glob", |b| {
		b.iter(|| bench_glob(black_box("/run/media/mmcblk0p1/steamapps/")))
	});
	c.bench_function("bench hashing", |b| {
		b.iter(|| bench_hashing(black_box("/run/media/mmcblk0p1/steamapps/")))
	});
}
