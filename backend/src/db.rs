...
pub fn get_id(table: &str, id: &str) -> Thing {
    Thing::from((table.to_string(), Id::from(id)))
}
...
pub async fn get_game(id: String) -> Result<Option<Game>, DynError> {
    Ok(crate::DB.select(("game", id.trim())).await?)
}

pub async fn get_card(id: String) -> Result<Option<MicroSDCard>, DynError> {
    Ok(crate::DB.select(("card", id.trim())).await?)
}
...
pub async fn add_game_to_card(game_id: String, card_id: String) -> Result<(), DynError>{
    ...
    let _ = crate::DB
        ...
        .bind(("card", get_id("card", card_id.trim())))
        .bind(("game", get_id("game", game_id.trim())))
        .await?;
...
pub async fn get_cards_with_games() -> Result<Vec<(MicroSDCard,Vec<Game>)>, DynError> {
    ...
    if games.len() != card.len() {
        ...
        return Err(Error::new_boxed(format!("Games and Cards did not match in count. Games: {}, Cards: {}", games.len(), card.len()).as_str()));
    }
